...
这是一组可复用的 Codex skills，用于澄清想法、压力测试计划、设计更有辨识度的前端、创作 algorithmic art、调研一手资料、保留写作声音、做决策、做更可靠的工程取舍，以及为项目建立长期可维护的 Codex 工作上下文。
...
Agent 很容易总结别人的总结，从而丢失事实来源。[`research`](./skills/research/SKILL.md) 会把阅读工作交给 background agent，优先检查 official docs、source code、specs 和 first-party APIs 等高可信一手资料，并把带引用的发现保存成 repo 里的 Markdown 文件。
...
- **[research](./skills/research/SKILL.md)** - 针对问题检查高可信一手资料，并把带引用的发现保存成 repo 里的 Markdown 文件。
